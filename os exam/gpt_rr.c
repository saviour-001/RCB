...
void roundRobin(struct Process p[], int n, int timeQuantum) {
    int currentTime = 0, completed = 0, allProcessesArrived = 0;
    
    while (completed < n) {
        int executedInCycle = 0;  // Flag to check if any process was executed in this cycle

        for (int i = 0; i < n; i++) {
            if (p[i].remainingTime > 0 && p[i].arrivalTime <= currentTime) {
                executedInCycle = 1; // Mark that at least one process was executed
        ...
        if (!executedInCycle) {
            currentTime++;  // If no process was executed, just increment time
        }
    }
}
